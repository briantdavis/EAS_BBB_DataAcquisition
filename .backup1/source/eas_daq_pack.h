...
// @@@@ friend EasDAQpack* BME280::fillEASpack(EasDAQpack &fillPack);
...
//
  private:
    // Sensor ID - identifies the sensor associated with the DAQ pack
    //             Sensor ID MUST be unique for each sensor or code
    //             region which generates DAQ pack (i.e. time stamp)
    uint8_t u_sensor_id;
  
    // Pack ID - identifies the 
...
//
  ...
  int setUnique(uint8_t);
  int setClockT(clock_t clk);
  int setClockDual(clock_t, struct timespec, uint8_t = UNSET_UID);
